...
\begin{itemize}
    ...
    \item \textbf{Steps:} 500,000 steps for all optimizers to evaluate long-term convergence and stability.
...
\caption{Training Loss over 500,000 steps. The Hybrid optimizer (Muon + AdamW) demonstrates superior convergence in the long run, achieving the lowest final loss.}
\label{fig:loss}
\end{figure}

The long-term training revealed that the \textbf{Hybrid approach} is particularly effective for full fine-tuning of Qwen2.5. By applying Muon's orthogonal updates to the early feature-extracting layers and using AdamW for higher-level logic, the model achieved a final training loss of approximately 2.15, compared to 2.73 for standard AdamW. 
MeZO, while extremely memory efficient, failed to show significant descent, confirming that zeroth-order methods require even larger step budgets for models of this scale.
